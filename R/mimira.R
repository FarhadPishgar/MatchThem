#' @title Multiply Imputed Repeated Analyses
#'
#' @docType class
#'
#' @aliases mimira
#'
#' @rdname mimira
#'
#' @name mimira
#'
#' @description \code{mimira} object contains data of multiply imputed repeated analyses. \code{mimira} objects are generated by calls to \code{with()}.
#'
#' @details \code{mimira} objects has methods for the \code{print()} and \code{summary()} functions (please see \pkg{mice} package reference manual for description of arguments of these functions).
#'
#' @note The \pkg{MatchThem} package does not use the S4 class definitions and instead relies on the S3 list equivalents.
#'
#' @seealso \code{\link[=with]{with}}
#'
#' @author Farhad Pishgar
#'
#' @references Stef van Buuren and Karin Groothuis-Oudshoorn (2011). \code{mice}: Multivariate Imputation by Chained Equations in \code{R}. \emph{Journal of Statistical Software}, 45(3): 1-67. \url{https://www.jstatsoft.org/v45/i03/}
#'
#' @keywords classes

NULL
