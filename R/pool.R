...
#'
#' @description \code{pool()} pools estimates from the analyses done within each imputed dataset. The typical sequence of steps to do a matching procedure on the imputed datasets are:
#' \enumerate{
...
#'
#' @return This function returns an object of the \code{mimipo} class. Methods for \code{mimipo} objects (e.g., \code{print()}, \code{summary()}, etc.) are available in \pkg{mice}, which does not need to be attached to use them.
#'
